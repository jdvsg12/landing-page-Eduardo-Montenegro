...
export function ServiceDetail({ service }: { service: Service }) {
    ...
    const whatsappHref =
        service.showWhatsapp && service.whatsapp
            ? `https://wa.me/${service.whatsapp.replace(/\D/g, "")}?text=${encodeURIComponent(waMessage)}`
            : null

    const primaryClass =
        "inline-flex min-h-11 items-center gap-3 bg-ink px-8 py-4 text-sm text-white transition-colors duration-200"
    const secondaryClass =
        "inline-flex min-h-11 items-center gap-3 border border-ink px-8 py-4 text-sm text-ink transition-colors duration-200 hover:bg-ink hover:text-white"

    const actions: { key: string; href: string; label: string; icon: ReactNode; className: string }[] = []
    if (whatsappHref) {
        actions.push({
            key: "whatsapp",
            href: whatsappHref,
            label: t.services.contactWhatsApp,
            icon: <WhatsAppIcon />,
            className: `${primaryClass} hover:bg-[#25D366] hover:text-[#0b2d18]`,
        })
    }
    if (service.showRegistration && service.registrationUrl) {
        actions.push({
            key: "registration",
            href: service.registrationUrl,
            label: pickLocale(service.registrationLabel, language) || t.services.detailRegistration,
            icon: null,
            className: actions.length > 0 ? secondaryClass : `${primaryClass} hover:bg-sage-deep`,
        })
    }
    if (service.showCalendar && service.calendarUrl) {
        actions.push({
            key: "calendar",
            href: service.calendarUrl,
            label: t.services.detailCalendar,
            icon: <CalendarIcon />,
            className: actions.length > 0 ? secondaryClass : `${primaryClass} hover:bg-sage-deep`,
        })
    }
        ...
        <div className="min-h-screen bg-paper text-neutral-900">
            ...
            <section className="mx-auto max-w-5xl px-6 pb-16 pt-32 lg:px-8 lg:pt-44">
                {kicker && (
                    <motion.p
                        {...enter(0)}
                        className="mb-8 text-xs uppercase tracking-[0.28em] text-sage-ink"
                    >
                        {kicker}
                    </motion.p>
                )}
                ...
                {excerpt && (
                    <>
                        <motion.div
                            {...enter(0.16)}
                            className="my-10 h-px w-full max-w-3xl bg-sage/50"
                        />
                        <motion.p
                            {...enter(0.24)}
            ...
            {service.coverImage && (
                <motion.figure
                    ...
                    className="relative mb-20 h-[45vh] w-full overflow-hidden md:h-[60vh]"
                >
                    <MediaImage
            ...
            {/* Cierre: las acciones que el servicio tenga activas, en una sola fila, y el formulario debajo */}
            <section className="mx-auto max-w-5xl px-6 pb-24 lg:px-8">
                <div className="max-w-2xl">
                {actions.length > 0 && (
                    <div className="border-t border-sage/50 pt-10">
                ...
                {service.showForm && (
                    <div className={actions.length > 0 ? "mt-16" : ""}>
