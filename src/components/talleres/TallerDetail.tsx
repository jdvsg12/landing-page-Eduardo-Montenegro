...
export function TallerDetail({ taller }: { taller: Taller }) {
  ...
  const whatsappHref = `https://wa.me/573142793431?text=${encodeURIComponent(
    t.talleres.whatsappMessage.replace("{title}", title),
  )}`
      ...
      <main id="main">
        <section className="relative flex min-h-[45vh] items-center bg-sage">
          ...
          <div className="relative z-10 mx-auto w-full max-w-4xl px-6 py-20 pt-32">
            ...
            <h1 className="mb-6 text-3xl font-light text-white md:text-5xl">{title}</h1>
            <div className="flex flex-wrap gap-3">
              <span className="inline-block bg-white/15 px-4 py-2 text-sm text-white">{formattedDate}</span>
              <span className="inline-block bg-white/15 px-4 py-2 text-sm text-white">{cost}</span>
            </div>
          </div>
        </section>

        <section className="mx-auto max-w-3xl px-6 py-16">
          <p className="text-xl leading-relaxed text-neutral-800">{excerpt}</p>

          <div className="mt-12 space-y-6">
            <PageContentBlocks
              blocks={taller.blocks}
              language={language}
              fallbackAlt={title}
              headingClassName="pt-4 font-serif text-2xl font-medium text-neutral-900"
              paragraphClassName="whitespace-pre-line text-base leading-relaxed text-neutral-700"
            />
          </div>

          {!taller.blocks.some((block) => block.type === "image") && taller.images.length > 0 && (
            <div className="mt-16 grid grid-cols-2 gap-4">
              {taller.images.map((img, index) => (
                <div key={index} className="relative aspect-[4/3] overflow-hidden">
                  <MediaImage src={img.url} alt={img.alt || title} sizes="40vw" />
                </div>
              ))}
            </div>
          )}
